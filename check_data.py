...
print(f"FMC file   : {FMC_PATH}")
print(
    f"Defect     : x={HOLE_X_MM - WIDTH_MM / 2:.2f} mm, depth={HOLE_Z_MM:.2f} mm, diameter={HOLE_D_MM:.2f} mm"
)
...
for i in range(N_TX):
    for j in range(N_RX):
        ...
        d_rx = (
            np.sqrt(
                (x_mm[np.newaxis, :] - elem_pos_mm[j]) ** 2 + z_mm[:, np.newaxis] ** 2
            )
            * 1e-3
        )  # m
        ...
        tfm += np.where(valid, a[kc] * (1.0 - frac) + a[kc + 1] * frac, np.float32(0.0))
...
im0 = axes[0].imshow(tfm_env, extent=extent, aspect="equal", cmap="hot", origin="upper")
...
plt.suptitle(
    f"TFM reconstruction  —  {N_TX}-element array @ {FREQ_MHZ:.0f} MHz, steel VL={VL:.0f} m/s",
    fontsize=11,
)
plt.tight_layout()
